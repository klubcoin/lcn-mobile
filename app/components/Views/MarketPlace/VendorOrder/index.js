	...
	renderItem = ({ item }) => {
		...
		return (
			<TouchableOpacity
				...
				onPress={() => this.onViewDetails({ order: item, amount: { total: amount, currencyUnit } })}
			>
				<View style={styles.storeNameContainer}>
					<View style={styles.storeNameAndIcon}>
						<Text style={styles.orderId}>
							#{orderId} - <Text style={styles.date}>{moment(createdAt).format('DD/MM/YY')}</Text>
						</Text>
					</View>
				...
				<View style={styles.customer}>
					...
					<Text style={styles.phone}>
						{strings('market.phone')}: {shipping.phone}
					</Text>
					<Text style={styles.address}>
						{strings('market.address')}: {shipping.address}
					</Text>
				</View>
	...
	renderOrderItems = () => {
		return (
			<View style={styles.body}>
				<FlatList data={this.orders} keyExtractor={item => item.uuid} renderItem={this.renderItem.bind(this)} />
			</View>
		);
	};
	...
	renderFilter = () => {
		return (
			...
			>
				...
				<TrackingScrollView style={{ flex: 1, marginBottom: 20 }}>
								...
								<View style={styles.sortOptionsWrapper}>
									{this.sortOptions.map((e, index) => (
										<TouchableOpacity
											style={[
												styles.sortOption,
												index == 1 && styles.middleOption,
												e.isSelected && styles.selectedOption
											]}
											onPress={() => this.onSelectSortOption(e.label)}
										>
