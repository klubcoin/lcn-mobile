...
const OngoingVote = ({ item, voteInstance, onTap }) => {
	...
	return (
		<TouchableOpacity style={styles.item} onPress={onTap} activeOpacity={0.6}>
			<RemoteImage fadeIn resizeMode="contain" source={{ uri: application?.iconUrl }} style={styles.avatar} />
			<Text style={styles.name}>{title}</Text>
			<Text style={styles.sectionIndex}>
				{strings('voting.section')} {index}
			</Text>
...
export class VotingApp extends PureComponent {
	...
	async fetchProposals() {
		...
		APIService.getVoteProposals(voteInstance.uuid, voterId, (success, json) => {
			this.proposals = [...json];
	...
	async fetchVotes() {
		...
		APIService.getVoteList(voteInstance.uuid, voterId, (success, json) => {
			this.votes = [...json];
			const currentTime = moment().unix();
			this.ongoingVotes = json.filter(
				e => e.voteStartDate.epochSecond < currentTime && currentTime < e.voteEndDate.epochSecond
			);
	...
	renderItem(item, section) {
		switch (section) {
			case 0:
				return (
					<OngoingVote item={item} voteInstance={this.voteInstance} onTap={() => this.openVote(item.item)} />
				);
			case 1:
				return (
					<ComingVote item={item} voteInstance={this.voteInstance} onTap={() => this.openVote(item.item)} />
				);
			case 2:
				return (
					<Proposal item={item} voteInstance={this.voteInstance} onTap={() => this.openProposal(item.item)} />
				);
			case 3:
				...
				<KeyboardAvoidingView style={{ flex: 1 }} behavior={'padding'} enabled={Device.isIos()}>
					...
					<TrackingScrollView contentContainerStyle={{ paddingBottom: 80 }}>
						<View style={styles.header}>
							<RemoteImage fadeIn resizeMode="contain" source={{ uri: iconUrl }} style={styles.logo} />
