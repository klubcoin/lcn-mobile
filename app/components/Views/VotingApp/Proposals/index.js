...
export class Proposals extends PureComponent {
	...
	openProposal(proposal) {
		this.props.navigation.navigate('VoteProposalDetails', { proposal, onUpdate: () => this.fetchProposals() });
	}
	...
	renderItem({ item }) {
		...
		return (
			<TouchableOpacity style={styles.item} activeOpacity={0.6} onPress={() => this.openProposal(item)}>
	...
	filterItems = () => {
		...
		return this.proposals.filter(e => {
			...
			return e.title.toLocaleLowerCase().includes(query) || date.toLocaleLowerCase().includes(query);
		});
	};
