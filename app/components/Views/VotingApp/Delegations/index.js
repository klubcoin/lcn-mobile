...
export class Delegations extends PureComponent {
	...
	editDelegation(delegation) {
		this.props.navigation.navigate('VoteDelegationAddEdit', {
			delegation,
			onUpdate: () => this.fetchDelegations()
		});
	}
	...
	openDelegation(delegation) {
		this.props.navigation.navigate('VoteDelegationDetails', {
			delegation,
			onUpdate: () => this.fetchDelegations()
		});
	}
	...
	renderActiveItem({ item }) {
		...
		return (
			<TouchableOpacity style={styles.item} activeOpacity={0.6} onPress={() => this.openDelegation(item)}>
				<RemoteImage fadeIn resizeMode="contain" source={{ uri: 'licoin.png' }} style={styles.avatar} />
				...
				<TouchableOpacity onPress={() => this.editDelegation(item)} style={styles.navButton}>
					<Icon name={'pencil'} size={16} style={styles.addIcon} />
				</TouchableOpacity>
				<TouchableOpacity onPress={() => this.deleteDelegation(item)} style={styles.navButton}>
					<Icon name={'trash'} size={16} style={styles.addIcon} />
				</TouchableOpacity>
			</TouchableOpacity>
		);
	}
	...
	filterItems = () => {
		...
		return this.delegations.filter(e => {
			...
			return e.category.toLocaleLowerCase().includes(query) || date.toLocaleLowerCase().includes(query);
		});
	};
			...
			<SafeAreaView>
				<View style={styles.navBar}>
					...
					<TouchableOpacity onPress={this.addDelegation.bind(this)} style={styles.navButton}>
						<Icon name={'plus'} size={16} style={styles.addIcon} color={colors.orange} />
					</TouchableOpacity>
				</View>
			</SafeAreaView>
		);
