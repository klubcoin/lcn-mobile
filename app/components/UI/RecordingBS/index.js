...
export default class RecordingBS extends Component {
    ...
    start = async () => {
        ...
        SoundRecorder.start(SoundRecorder.PATH_CACHE + `/${new Date().getTime().toString()}.mp4`, Platform.OS == 'ios' ? null : { format: 2, encoder: 4 })
            .then(function () {
        ...
        if (recording) {
            ...
            return (
                <TouchableOpacity style={styles.micWrapper} onPress={this.stop}>
                    <Animated.View style={[styles.micButtonCircle, styles.micOverlay, {
                        opacity: opacity,
                        transform: [
                            {
                                scale: animated
        ...
        return (
            ...
            >
                <View style={styles.container}>
                    ...
                    {this._micButton()}
                    <TouchableOpacity disabled={!audioFile} onPress={paused ? this.play : this.pause} {...testID('play-button')} style={{marginTop: 30}}>
                        <Icon name={paused ? "play" : "pause"} size={30} color={audioFile ? colors.primary : colors.lightGray} />
                    </TouchableOpacity>
                    <StyledButton
                        type={'confirm'}
                        onPress={() => {
